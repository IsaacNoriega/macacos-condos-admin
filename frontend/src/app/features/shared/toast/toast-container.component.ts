  ...
  template: `
    <div class="mcs-toast-stack" aria-live="polite" aria-atomic="true">
      @for (t of toast.toasts(); track t.id) {
        <div class="mcs-toast" [class.ok]="t.kind === 'ok'" [class.bad]="t.kind === 'bad'" [class.info]="t.kind === 'info'">
          <span class="mcs-toast-icon">
            @switch (t.kind) {
              @case ('ok')   { <mac-icon name="checkCircle" [size]="18"/> }
              @case ('bad')  { <mac-icon name="xCircle"     [size]="18"/> }
              @case ('info') { <mac-icon name="info"        [size]="18"/> }
            }
          </span>
          <div style="flex:1; min-width:0;">
            <div class="mcs-toast-title">{{ t.title }}</div>
            @if (t.msg) { <div class="mcs-toast-msg">{{ t.msg }}</div> }
          </div>
          <button type="button" class="mcs-btn ghost sm icon-only" (click)="toast.dismiss(t.id)" aria-label="Cerrar aviso">
